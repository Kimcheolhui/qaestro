...
devclaw의 구현 언어는 `Python`을 기준으로 한다.
...
devclaw의 1차 agent execution framework는 `Microsoft Agent Framework`를 사용한다.

Microsoft Agent Framework는 agent 실행, tool 호출, streaming, hosting을 담당하고, 이벤트 해석, workflow 제어, validation sequencing 같은 도메인 로직은 devclaw의 `runtime/orchestrator`가 계속 책임진다.
