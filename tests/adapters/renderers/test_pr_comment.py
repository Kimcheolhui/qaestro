...
def test_github_pr_comment_renderer_turns_qa_report_into_pr_comment_payload():
    ...
    assert "Outcome: **SKIPPED**" in payload.body
...
def test_pr_comment_renderer_includes_structured_validation_evidence() -> None:
    action = StrategyAction(
        action_type=ActionType.VERIFY_API_CONTRACT,
        description="Probe read-only pull request endpoint",
        target="GET /pull-request",
        priority=2,
        rationale="Runtime validation should prove API contract visibility.",
    )
    report = QAReport(
        event_id="evt-validation",
        repo_full_name="Kimcheolhui/qaestro-test",
        pr_number=12,
        impact=BehaviourImpact(summary="Runtime validation smoke", areas=(), overall_risk=RiskLevel.MEDIUM),
        strategy=StrategyResult(actions=(action,), reasoning="API contract changed.", confidence=0.8),
        validations=(
            ValidationResult(
                action=action,
                outcome=ValidationOutcome.PASS,
                details="contract_ok: response schema matched",
                duration_seconds=1.234,
                artifacts=("https://example.test/artifacts/validation.json",),
            ),
        ),
        summary_markdown="Runtime validation smoke",
    )

    payload = GitHubPRCommentRenderer().render(report, correlation_id="corr-validation")

    assert "### Validation" in payload.body
    assert "VERIFY API CONTRACT" in payload.body
    assert "Probe read-only pull request endpoint" in payload.body
    assert "Target: `GET /pull-request`" in payload.body
